...
# اگر روی Railway یک Volume وصل کرده باشید، مسیرش در RAILWAY_VOLUME_MOUNT_PATH است؛
# دیتابیس آنجا ذخیره می‌شود تا با هر Deploy جدید پاک نشود. در غیر این صورت کنار پروژه ذخیره می‌شود
# (روی Railway بدون Volume، فایل‌سیستم موقت است و با هر Deploy از نو می‌شود).
_volume = os.getenv("RAILWAY_VOLUME_MOUNT_PATH")
DB_PATH = os.getenv("DB_PATH") or (str(Path(_volume) / "racing.db") if _volume else "racing.db")
